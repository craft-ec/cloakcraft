...
pub fn create_pending_with_proof_open_position<'info>(
    ...
    // Store output commitment (position)
    ...
    // Store extra data for position (using extra_amount for position size calculation)
    // Position size = margin * leverage (calculated in execute phase with oracle price)
    pending_op.extra_amount = 0; // Will be set in execute phase
