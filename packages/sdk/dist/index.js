...
async function buildInitializeAmmPoolWithProgram(program, params) {
  ...
  const tx = await program.methods.initializeAmmPool(
    ...
    params.feeBps,
    poolTypeEnum,
    new import_anchor2.BN(amplification)
  ).accountsStrict({
    ammPool: ammPoolPda,
...
function derivePerpsPoolPda(poolId, programId = PROGRAM_ID) {
  return import_web313.PublicKey.findProgramAddressSync(
...
async function buildOpenPositionWithProgram(program, params) {
  ...
  const phase0Tx = await program.methods.createPendingWithProofOpenPosition(
    ...
    params.isLong,
    new import_anchor6.BN(params.marginAmount.toString()),
    params.leverage,
    new import_anchor6.BN(params.positionFee.toString())
  ).accountsStrict({
    ...
    import_web313.ComputeBudgetProgram.setComputeUnitLimit({ units: 8e5 })
  ]);
  const phase1Tx = await program.methods.verifyCommitmentExists(
    Array.from(operationId),
    0,
    ...
    import_web313.ComputeBudgetProgram.setComputeUnitLimit({ units: 4e5 })
  ]);
  const phase2Tx = await program.methods.createNullifierAndPending(
    Array.from(operationId),
    0,
  ...
  const phase3Tx = await program.methods.executeOpenPosition(
    Array.from(operationId),
    new import_anchor6.BN(params.entryPrice.toString())
  ).accountsStrict({
    settlementPool: params.settlementPool,
    perpsPool: params.perpsPool,
    perpsMarket: params.market,
...
async function buildClosePositionWithProgram(program, params) {
  ...
  const phase0Tx = await program.methods.createPendingWithProofClosePosition(
    ...
    params.isLong,
    new import_anchor6.BN(params.exitPrice.toString()),
    new import_anchor6.BN(params.closeFee.toString()),
    new import_anchor6.BN(params.pnlAmount.toString()),
    params.isProfit
  ).accountsStrict({
    settlementPool: params.settlementPool,
    perpsPool: params.perpsPool,
  ...
  const phase3Tx = await program.methods.executeClosePosition(
    Array.from(operationId),
    new import_anchor6.BN(params.positionMargin.toString()),
    new import_anchor6.BN(params.positionSize.toString()),
    new import_anchor6.BN(params.entryPrice.toString())
  ).accountsStrict({
    settlementPool: params.settlementPool,
    perpsPool: params.perpsPool,
...
async function buildAddPerpsLiquidityWithProgram(program, params) {
  ...
  for (let i = 0; i < 8; i++) {
    oraclePricesBN.push(new import_anchor6.BN((params.oraclePrices[i] ?? 0n).toString()));
  }
  const phase0Tx = await program.methods.createPendingWithProofAddPerpsLiquidity(
    ...
    Array.from(params.lpCommitment),
    params.tokenIndex,
    new import_anchor6.BN(params.depositAmount.toString()),
    new import_anchor6.BN(params.lpAmountMinted.toString()),
    new import_anchor6.BN(params.feeAmount.toString())
  ).accountsStrict({
    settlementPool: params.settlementPool,
    perpsPool: params.perpsPool,
    verificationKey: vkPda,
...
async function buildRemovePerpsLiquidityWithProgram(program, params) {
  ...
  const phase0Tx = await program.methods.createPendingWithProofRemovePerpsLiquidity(
    ...
    params.tokenIndex,
    new import_anchor6.BN(params.withdrawAmount.toString()),
    new import_anchor6.BN(params.lpAmountBurned.toString()),
    new import_anchor6.BN(params.feeAmount.toString())
  ).accountsStrict({
    settlementPool: params.settlementPool,
    perpsPool: params.perpsPool,
...
async function buildAddMarketWithProgram(program, params) {
  ...
  const tx = await program.methods.addMarket(
    ...
    new import_anchor6.BN(params.maxPositionSize.toString())
  ).accountsStrict({
    perpsPool: params.perpsPool,
...
var CloakCraftClient = class {
  ...
  async transfer(params, relayer) {
    ...
    if (params.fee && params.fee > 0n) {
      ...
      if (feeConfig?.treasury) {
        ...
        treasuryTokenAccount = (0, import_spl_token6.getAssociatedTokenAddressSync)(
          tokenMint,
          treasuryWallet,
          true
          // allowOwnerOffCurve
  ...
  async submitVote(params, relayer) {
    ...
    const tokenMint = params.input.tokenMint instanceof Uint8Array ? new import_web316.PublicKey(params.input.tokenMint) : params.input.tokenMint;
