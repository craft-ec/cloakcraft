...
var init_constants = __esm({
  "src/instructions/constants.ts"() {
    ...
    PROGRAM_ID = new import_web34.PublicKey("DsCP619hPxpvY1SKfCqoKMB7om52UJBKBewevvoNN7Ha");
    SEEDS = {
...
var import_web35, import_stateless2, LightProtocol;
  ...
  "src/instructions/light-helpers.ts"() {
    ...
    LightProtocol = class {
      ...
      async getInclusionProofByHash(accountHash) {
        const hashBytes = new import_web35.PublicKey(accountHash).toBytes();
      ...
      buildRemainingAccountsWithCommitment(commitmentProof, nullifierProof) {
        ...
        const commitmentTree = new import_web35.PublicKey(commitmentProof.treeInfo.tree);
        const commitmentQueue = new import_web35.PublicKey(commitmentProof.treeInfo.queue);
...
var import_web36, import_spl_token, import_anchor;
...
async function buildConsolidationWithProgram(program, params, rpcUrl) {
  ...
  const inputProofs = await Promise.all(
    params.inputs.map(async (input, i) => {
      ...
      if (commitmentProof.treeInfo.cpiContext) {
        packedAccounts.insertOrGet(new import_web38.PublicKey(commitmentProof.treeInfo.cpiContext));
      }
      return {
        commitmentProof,
        treeIndex,
  ...
  const phase0Tx = await program.methods.createPendingWithProofConsolidation(
    ...
    // output_recipient
    new import_anchor3.BN(params.outputAmount.toString()),
    // output_amount
  ...
  const phase1Txs = await Promise.all(
    params.inputs.map(async (input, i) => {
      ...
      const lightParams = {
        commitmentAccountHash: Array.from(new import_web38.PublicKey(input.accountHash).toBytes()),
        commitmentMerkleContext: {
...
async function buildCreateAggregationWithProgram(program, params) {
  ...
  const tx = await program.methods.createAggregation(
    ...
    authority: params.authority,
    payer: params.payer,
    systemProgram: import_web312.PublicKey.default
  }).preInstructions([
    import_web312.ComputeBudgetProgram.setComputeUnitLimit({ units: 2e5 })
  ]);
  return tx;
}
async function buildSubmitVoteWithProgram(program, params, rpcUrl) {
  ...
  const tx = await program.methods.submitEncrypted(
    ...
    import_web312.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 5e4 })
  ]);
  return tx;
}
...
var CloakCraftClient = class {
  ...
  async submitVote(params, relayer) {
    ...
    const tokenMint = params.input.tokenMint instanceof Uint8Array ? new import_web315.PublicKey(params.input.tokenMint) : params.input.tokenMint;
